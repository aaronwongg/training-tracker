...
- **Climbs** (bike): highlight notable climbs from a ride — name, length (mi), avg grade (%), and elevation gain (ft). Leave the grade blank and the placeholder suggests it from length + gain. Climbs are per-ride results: they don't carry forward to next week and aren't stored in saved workouts, but they do appear in the export.
